...
const RepositoryModal = () => {
  ...
  const clickRepoEvent = (e: React.MouseEvent<HTMLLIElement>) => {
    ...
    localStorage.setItem(
      "repo",
      JSON.stringify(repos!.find((repo: any) => repo.name === id))
    );
  };

  const getRepositoryList = async () => {
    try {
      await fetch(`/api/repos`).then((res) => {
        res.json().then((data) => {
          setRepos(data);
        });
      });
  ...
  return (
    <>
      <FaExchangeAlt
        onClick={resetRepo}
        className="size-7 rounded-xl cursor-pointer hover:bg-gray-100 p-1"
      />
      {isOpen ? (
                ...
                <ul className="w-full flex flex-col gap-2">
                  {repos.map((repo, index) => (
                    ...
                    >
                      <li
                        id={repo.name}
                        className="cursor-pointer hover:bg-gray-200 py-2 px-3 font-bold flex items-center justify-between rounded-lg bg-gray-100"
